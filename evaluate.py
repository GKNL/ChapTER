...
from input_temporal.doc_temporal import load_data, Example
from predict import BertPredictor
from input_temporal.dict_hub_temporal import get_entity_dict, get_all_filter_quadruple_dict
...
all_filter_quadruple_dict = get_all_filter_quadruple_dict()
...
def compute_metrics(hr_tensor: torch.tensor,
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        #     gold_neighbor_ids = all_filter_quadruple_dict.get_neighbors(cur_ex.head_id, cur_ex.relation, cur_ex.time)  # time filter
...
def eval_single_direction(predictor: BertPredictor,
                          ...
                          batch_size=256) -> dict:
    start_time = time()
    examples = load_data(args.valid_path, add_forward_quadruple=eval_forward, add_backward_quadruple=not eval_forward)
