...
parser = argparse.ArgumentParser(description='ChapTER arguments')
...
# For p-tuning
parser.add_argument('--prompt_length', default=20, type=int,
                    help='length of prefix prompt')
parser.add_argument('--prompt_emb_dim', default=164, type=int,
                    help='initial embedding dimension of prompt')
parser.add_argument('--prompt_hidden_dim', default=-1, type=int,
                    help='hidden dimension of prompt')
parser.add_argument('--prefix_projection', action='store_true',
                    help='Use a two-layer MLP to encode the prefix')

# For temporal
parser.add_argument('--use-timehistory-graph', action='store_true',
                    help='use historical entities from timehistory graph as context')
...
if args.prompt_hidden_dim == -1:
    args.prompt_hidden_dim = args.prompt_emb_dim // 2

assert not args.train_path or os.path.exists(args.train_path)
assert args.pooling in ['cls', 'mean', 'max']
assert args.task.lower() in ['icews18', 'icews14', 'icews0515', 'icews14f']
